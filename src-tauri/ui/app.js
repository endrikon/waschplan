...
const addHoliday = document.querySelector("#addHoliday");
...
function makeHoliday(name, date) {
...
document.addEventListener('click', (e) => {
  ...
  if (target) {
    ...
    datePicker.bulmaCalendar.datePicker.disabledDates =
      datePicker.bulmaCalendar.datePicker.disabledDates.filter(d => d.toLocaleDateString() !== dateObj);
    parent.remove();
  }
});
...
const dateDeleteBtn = document.querySelector('.datetimepicker-clear-button');
